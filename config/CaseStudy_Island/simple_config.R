...
divergence_threshold = runif(1,min=5, max=5.5)  ### Arbitrary value to ensure enough diversification wihtout stopping the simulation midway
...
apply_evolution <- function(species, cluster_indices, landscape, config) {
  ...
  traits[, "temp"] <- traits[, "temp"] + rnorm(length(traits[, "temp"]), mean = 0, sd = 0.3) # sd value: control = 0.5; less phylo constraint=0.7; higher phylo constraint=0.3 
  traits[, "prec"] <- traits[, "prec"] + rnorm(length(traits[, "prec"]), mean = 0, sd = 30)  # sd value: control = 50; less phylo constraint=70; higher phylo constraint=30
  # your mutations
  return(traits)
}
...
apply_ecology <- function(abundance, traits, landscape, config) {
  abundance <- ( abs( traits[, "temp"] - landscape[, "temp"]) < 1.5 ) &
    ( abs( traits[, "prec"] - landscape[, "prec"]) < 150 )
  return(abundance)
}
