...
#'
#' @export
mutation_mode_adaptive <- function(evolving_traits,
                               trait_rules,
                               mutation_factor,
  ...
  mutation_mode_adaptive_internal <- function(species, cluster_indices, landscape, config) {
    ...
    if (any(mutations)) { # if mutations hapened, then...
      ...
      mutation_deltas <-rnorm(num_mutation_values, mean=0, sd=trait_evolutionary_power)

      temp_idi <- names(species$abundance)
      t_env <- landscape$environment[temp_idi, "temp"]
      t_opt <- NEW_clu_tr_spi[,"t_min"]+0.05      # 0.05 = 1/2 rt
      t_dif <- t_env - t_opt
      evo_power <- abs(rnorm(1, mean=0, sd=trait_evolutionary_power))
      evo_power <- ifelse(t_dif>=0,pmin(evo_power,t_dif),pmax(-evo_power,t_dif))
      mutation_deltas <- (mutations * evo_power)[mutations]
    ...
    species_traits <- NEW_clu_tr_spi
    return(invisible(species_traits))
  }
  return(invisible(mutation_mode_adaptive_internal))
}
